...
'''
Read the flight operation text data and perform slicing processing.
The slicing range is from 6 to 95.
Extract data from different files to build a dataset and labels.
Save all the extracted data to the corresponding files in the processData\\dealFlightData directory.
'''
...
class ReadDataDemo():
    ...
    # Get the number of targets and labels in the file
    ...
    def slice_and_process_file(self, input_file, file_name):
        try:
            ...
            output_file = os.path.join('..\\processData\\dealFlightData', f"{base_name}_processed{ext}")

            eye_ = self.load_eye_txt(input_file)

            # Get the number of targets in the file
...
if __name__ == '__main__':
    ...
    txt_files, txt_name = readdata.get_txt_files('..\\rowData\\flight_data')
